...
/**
 * Retorna a linhagem completa de uma classe, da raiz até a classe atual.
 * Ex.: 'lord_knight' -> ['swordman', 'knight', 'lord_knight'].
 * @param {string} classId
 * @returns {string[]}
 */
export function getClassLineage(classId) {
    if (!classId || !JOBS_META[classId]) return [];

    const lineage = [];
    let current = classId;

    while (current && JOBS_META[current]) {
        lineage.unshift(current);
        current = JOBS_META[current].reqJob ?? null;
    }
